...
import { getAllMessages } from '../../services/HallServices';
import ArticleModel from "../ArticleModel/ArticleModel";
import LoadingPage from "../Loading/LoadingPage";

const NewlyDepositedMessages = () => {
    const [property, setPropertity] = useState([]);
    const [refresh, setRefresh] = useState(false);

    useEffect(() => {
        getAllMessages().then((res) => {
            setPropertity(res.data.article.slice(-6));
        })
    }, [refresh]);
    return (
        <>
                                ...
                                property ?
                                    property?.map((item, index) => (
                                        <WrapItem key={index} dir="rtl" >
                                            <ArticleModel item={item} hallid={item._id} refresh={refresh} setRefresh={setRefresh} />
                                        </WrapItem>
                                    )) : (<LoadingPage />)
                            }
                        </Wrap>
...
export default NewlyDepositedMessages;