...
//libarary
export const getCommentByBookId = async (id) => {
    const { data } = await axios.get(`http://localhost:5000/api/v1/comments/books/${id}`,config);
    return data;
};

//libarary
export const getCommentByArticleId = async (id) => {
    const { data } = await axios.get(`http://localhost:5000/api/v1/comments/articls/${id}`,config);
    return data;
};
