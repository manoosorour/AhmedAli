...
// libarary
export const getRatingByBookId = async (productId) => {
    const { data } = await axios.get(`http://localhost:5000/api/v1/ratings/books/${productId}`,config);
    return data;
}

// libarary
export const getRatingByArticleId = async (productId) => {
    const { data } = await axios.get(`http://localhost:5000/api/v1/ratings/articls/${productId}`,config);
    return data;
}
